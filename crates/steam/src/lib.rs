...
pub fn init_steamworks() -> Result<(), SteamError> {
    ...
    tracing::info!("Initialized steamworks");
...
pub fn begin_session(
    ...
    tracing::debug!("Starting steam session for {:?}", steam_id);
    ...
    if let Err(error) = result {
        match error {
            steamworks::AuthSessionError::DuplicateRequest => {
                tracing::warn!("Got duplicate request, attempting to clear the previous one");
                STEAM_SERVER.get()
                    .expect("Could not get steam API instace")
                ...
                tracing::warn!("Reattempting the auth");
            },
...
impl Drop for SteamSession {
    fn drop(&mut self) {
        tracing::debug!("Ending steam session for {:?}", self);
