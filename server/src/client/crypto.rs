...
impl ClientCrypto<ClientCryptoStateCreated> {
    pub fn generate_new(self) -> ClientCrypto<ClientCryptoStateParametersGenerated> {
        ...
        unsafe {
            randombytes_buf(server_nonce.as_mut_ptr() as *mut ffi::c_void, NONCEBYTES);
            randombytes_buf(client_nonce.as_mut_ptr() as *mut ffi::c_void, NONCEBYTES);
        };
...
impl ClientCrypto<ClientCryptoStateParametersGenerated> {
    ...
    pub async fn kx_decrypt<R: Read>(&mut self, r: R) -> Result<Vec<u8>, ClientError> {
        ...
        let result = unsafe {
            libsodium_sys::crypto_box_open_detached(
                ...
                payload.len() as u64,
                nonce.as_ptr(),
                key::CLIENT_PUBLIC_KEY.as_ptr(),
                key::SERVER_SECRET_KEY.as_ptr(),
            )
        };
    ...
    pub async fn kx_encrypt(
        ...
        unsafe {
            randombytes_buf(bootstrap_nonce.as_mut_ptr() as *mut ffi::c_void, NONCEBYTES);
        };
